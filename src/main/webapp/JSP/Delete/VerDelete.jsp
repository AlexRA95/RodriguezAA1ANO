    ...
    <div class="container">
        <div class="row">
            <div class="col-12 text-center">
                <h1 class="display-1">Selecciona el servicio a borrar</h1>
            </div>
        </div>
        <div class="row">
            <form action="${applicationScope.contexto}/Delete" method="post" class="col-12 d-flex justify-content-center flex-column">
                        ...
                        <c:otherwise>
                            <c:forEach items="${requestScope.servicios}" var="servicio" varStatus="status">
                                <tr>
                                    ...
                                    <td>${servicio.path}</td>
                                    <td><input class="form-check-input" type="radio" name="servDelete" value="${servicio.id}" ${status.first ? 'checked' : ''}> </td>
                                </tr>
                            </c:forEach>
                        </c:otherwise>
                    </c:choose>
                ...
                <button type="submit" name="opcion" value="verDelete" class="btn btn-primary">Seleccionar</button>
            </form>
        </div>
    </div>
</main>
